...
(function (global, factory) {
	...
	var show = function show(stepperNode, indexStep, options, done) {
		...
		if (
			stepper._steps[indexStep].classList.contains(ClassName.ACTIVE) ||
			stepper._stepsContents[indexStep].classList.contains(ClassName.ACTIVE)
		) {
			return;
		}
		...
		if (activeContent.length) {
			...
			if (
				!stepperNode.classList.contains(ClassName.VERTICAL) &&
				!stepper.options.animation
			) {
		...
		showContent(
			stepperNode,
			stepper._stepsContents[indexStep],
			stepper._stepsContents,
			activeContent,
			done
		);
	};
	...
	var buildClickStepNonLinearListener =
	...
	var Stepper =
		...
		(function () {
			function Stepper(element, _options) {
				...
				this.options.selectors = _extends(
					{},
				...
				this._steps
					...
					.forEach(function (step) {
						_this._stepsContents.push(
							_this._element.querySelector(step.getAttribute("data-target"))
						);
					});
			...
			_proto._setLinkListeners = function _setLinkListeners() {
				...
				this._steps.forEach(function (step) {
					...
					if (_this2.options.linear) {
						_this2._clickStepLinearListener = buildClickStepLinearListener(
							_this2.options
						);
						trigger.addEventListener("click", _this2._clickStepLinearListener);
					} else {
						_this2._clickStepNonLinearListener =
							buildClickStepNonLinearListener(_this2.options);
			...
			_proto.to = function to(stepNumber) {
				...
				var nextStep =
					tempIndex >= 0 && tempIndex < this._steps.length ? tempIndex : 0;
				show(this._element, nextStep, this.options, function () {
				...
				this._steps.forEach(function (step) {
					...
					if (_this7.options.linear) {
						trigger.removeEventListener(
							"click",
							_this7._clickStepLinearListener
						);
					} else {
						trigger.removeEventListener(
							"click",
							_this7._clickStepNonLinearListener
						);
					}
				});
